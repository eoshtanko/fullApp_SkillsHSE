...
class Can extends StatefulWidget {
  static of(BuildContext context, {bool root = false}) => root
      ? context.findRootAncestorStateOfType<CanState>()
      : context.findAncestorStateOfType<CanState>();

  Can(List<Skill> data, Profile prof) {
    skillsCan = data;
    profile = prof;
  }
...
class CanState extends State<Can> {
  ...
  void loadSkills() {
    Api.getCanSkills(profile).then((value) => {
      if (value.isSuccess()) {skillsCan = value.getData(), setState(() {})}
    });
  }
...
DialogLoading dialogLoading;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text('Может', style: TextStyle(color: Colors.blue)),
